...
## Subtasks
- [ ] 04.1 Define the `grade_round` and `grader_verdict` event payloads.
- [ ] 04.2 Add the `Grade` lifecycle key and `GradeLoop` item kind with their match arms.
- [ ] 04.3 Register a `grade_round` dispatch arm before the catch-all in `apply_history_event`.
- [ ] 04.4 Implement `apply_grade_round` to read-prior-then-accumulate under the `Grade` key.
- [ ] 04.5 Map outcomes to status/severity and bound the body to 12 lines.
- [ ] 04.6 Cover collapse, counter, status transitions, and skip rendering with tests.
