...
## Subtasks
- [ ] 3.1 Add the reserved-key guard at the top of `key_event_to_tui_command_with_ui`.
- [ ] 3.2 Remove the now-redundant `Ctrl-C` branches at the three sites.
- [ ] 3.3 Confirm behavior is unchanged across all modal contexts.
- [ ] 3.4 Add a regression test asserting `Ctrl-C` → interrupt in every context.
...
## Tests
- Unit tests:
  - [ ] `key_event_to_tui_command_with_ui` returns `DispatchAndQuit(RunInterruptRequested)` for `Ctrl-C` when help is visible.
  - [ ] Same for clarification pending, approval pending, an active dropdown, and plain normal input (table across contexts).
  - [ ] A non-`Ctrl-C` key in each of those contexts is unaffected (routes as before).
- Integration tests:
  - [ ] Existing app/run-loop tests that depend on `Ctrl-C` interrupting an active run still pass unchanged.
